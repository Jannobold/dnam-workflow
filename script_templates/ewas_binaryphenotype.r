...
for(i in 1:nrow(dnam)){
  ...
  testdata$BINARY_VARIABLE <- as.factor(testdata$BINARY_VARIABLE)
  #[DNAM PCS HERE] - e.g., PC1 + PC2 + PC3 etc., the number of PCs that should be included can be determined by a scree plot
  #BINARY VARIABLE is a variable which contains two values (e.g. yes / no, or smoking status current / never, etc.)
  linreg_m <- lm(formula = probe ~ BINARY_VARIABLE + Age_T1 + [DNAM PCS HERE], data=males)
  linreg_f <- lm(formula = probe ~ BINARY_VARIABLE + Age_T1 + [DNAM PCS HERE], data=females)
  linreg <- lm(formula = probe ~ BINARY_VARIABLE + geschlecht + Age_T1 + [DNAM PCS HERE], data=testdata)
