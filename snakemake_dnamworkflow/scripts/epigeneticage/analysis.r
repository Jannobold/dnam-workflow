...
#YS: change stop to cat(paste()) and append to logfile to continue with analysis
if ( sum( selectCpGsClock) < dim(datClock)[[1]]-1 ) {cat(paste("The CpGs listed in column 1 of the input data did not contain the CpGs needed for calculating DNAm age. Make sure to input cg numbers such as cg00075967."), file="LogFile.txt",append=TRUE)}
...
#YS: filter datClock for those CpGs that are present in the data
temp <- datClock[datClock$CpGmarker %in% colnames(datMethClock0), ]
datClock <- rbind(datClock[1, ], temp)

#YS: added a comma [ ,as.character...] to specify columns
